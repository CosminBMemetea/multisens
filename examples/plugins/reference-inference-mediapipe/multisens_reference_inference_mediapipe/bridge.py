"""A thin `PredictionConnector` bridge - third sibling to
multisens_reference_inference's own yolo_bridge and
multisens_reference_inference_emotion's own emotion_bridge, byte-for-byte
the same architecture: polls a separately-running inference worker
process's small local HTTP endpoint (see the sibling `worker/`
directory) and translates its latest detection into canonical
`Prediction` objects. Owns zero ML dependency itself - MediaPipe and its
RTSP reader live in a genuinely separate OS process, so a native-level
crash there can never take down this plugin or the MultiSens backend
process it's loaded into.

Staleness handling, error logging, dedup-on-unchanged-timestamp: all
identical reasoning to yolo_bridge.py/emotion_bridge.py (issues
#126/#127) - not re-derived here, just re-applied a third time,
confirming it generalizes.

Face *detection* only, not classification - `top_emotion` from the
sibling emotion bridge has no equivalent here on purpose. `face_count`
(not just `face_present`) reflects that this is a genuine multi-face
model, unlike the single-face-scoped emotion worker.
"""
...
PLUGIN_ID = 'multisens.reference.inference.mediapipe_bridge'
HOMEPAGE = 'https://github.com/CosminBMemetea/multisens/tree/main/examples/plugins/reference-inference-mediapipe'

SUPPORTED_MODALITIES = ('rgb',)
DEFAULT_TASK = 'face_detection'
DEFAULT_TIMEOUT_S = 2.0
DEFAULT_STALE_AFTER_S = 5.0


class MediaPipeBridgeConnector:
    def __init__(self) -> None:
        ...
        # detections - derived from the same already-filtered list
        # poll() already emits, no second copy of any threshold logic.
        self._last_detections: list[dict[str, Any]] = []

    def descriptor(self) -> PluginDescriptor:
        return PluginDescriptor(
            plugin_id=PLUGIN_ID, name='MediaPipe Face Detection Bridge', version='0.1.0',
            ...
            description=(
                'Thin bridge to a separately-running MediaPipe face-detection worker (see the '
                'sibling worker/ directory) - zero ML dependency itself, process-isolated from the '
                'actual model. Detection only, not classification.'
            ),
        )
    ...
    def poll(self) -> list[Prediction]:
        ...
        try:
            ...
            print(f'mediapipe_bridge: sensor_id={self._sensor_id!r} poll failed: {type(e).__name__}: {e}')
            raise
        self._last_error = None
        ...
        return [Prediction(
            ...
            source_id=f'{self._sensor_id}.mediapipe_bridge', sensor_ids=[self._sensor_id],
