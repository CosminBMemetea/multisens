"""Thread-safe shared state between the capture/inference thread and
the HTTP server thread - the worker's own internal producer/consumer
boundary, deliberately tiny (a lock-guarded snapshot, nothing
event-driven) since the HTTP side only ever needs "whatever the latest
finished frame was," never a queue of history.
"""
