"""The worker's small local HTTP endpoint - stdlib `http.server` only,
no web framework dependency, since this never needs to be more than
`GET /latest` and `GET /health`. Deliberately never imports `capture.py`
(the only module with `cv2`/`ultralytics` dependencies), so this module
