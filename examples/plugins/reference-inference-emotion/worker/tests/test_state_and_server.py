...
onnxruntime/numpy (server.py deliberately never imports capture.py, the
only module that needs those). Drives a real HTTP server on a random
local port and makes real GET requests, mirroring yolo_worker's own
test_state_and_server.py exactly - state.py/server.py are generic, this
suite is nearly a byte-for-byte port."""
import json
import threading
import urllib.error
