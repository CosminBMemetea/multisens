"""Phase 88 (v0.8): the shipped PropertyWatch object-detection demo's
numbers must not silently drift, same discipline as Phase 87's own
RideSafe detection guard (test_ridesafe_detection_demo.py) - the IoU/
greedy-matching reimplementation below is deliberately duplicated rather
than imported from that file, so this test file stands on its own with
zero cross-test-file coupling, matching how every prior demo guard in
this project is self-contained.

Independently recomputes IoU, greedy per-frame object matching, and
session-level TP/FP/FN/precision/recall/mean-matched-IoU straight from
the raw JSON - zero imports from app.domain.detection - and cross-checks
against the real /evaluate API end to end.
"""
import json
...
DATA_PATH = Path(__file__).resolve().parents[2] / 'examples' / 'profiles' / 'propertywatch-detection-demo-data.json'
...
TASK_CONFIGS = {
    'entrance_package_detection': 'cfg-property_entrance_rgb',
    'storage_object_detection': 'cfg-property_storage_rgb',
    'indoor_object_detection': 'cfg-property_indoor_rgb',
}

TASK_LABELS = {
    'entrance_package_detection': {'package', 'person'},
    'storage_object_detection': {'vehicle', 'person'},
    'indoor_object_detection': {'person', 'pet'},
}

# By construction (see scripts/generate_propertywatch_detection_demo_data.py's
# own docstring), not measured.
EXPECTED = {
    'entrance_package_detection': {
        'true_positives': 80, 'false_positives': 15, 'false_negatives': 20,
        'precision': 80 / 95, 'recall': 0.80, 'f1': 2 * (80 / 95) * 0.80 / ((80 / 95) + 0.80),
        'mean_iou_matched': 0.625,
    },
    'storage_object_detection': {
        'true_positives': 60, 'false_positives': 20, 'false_negatives': 40,
        'precision': 0.75, 'recall': 0.60, 'f1': 2 * 0.75 * 0.60 / (0.75 + 0.60),
        'mean_iou_matched': 38 / 60,
    },
    'indoor_object_detection': {
        'true_positives': 45, 'false_positives': 25, 'false_negatives': 55,
        'precision': 45 / 70, 'recall': 0.45, 'f1': 2 * (45 / 70) * 0.45 / ((45 / 70) + 0.45),
        'mean_iou_matched': 29 / 45,
    },
}
...
# Prediction row per GT row. a+b+c+e / d per task.
EXPECTED_FRAME_COUNTS = {
    'entrance_package_detection': {'matched_samples': 95, 'unmatched_ground_truth': 5, 'unmatched_predictions': 0},
    'storage_object_detection': {'matched_samples': 85, 'unmatched_ground_truth': 15, 'unmatched_predictions': 0},
    'indoor_object_detection': {'matched_samples': 80, 'unmatched_ground_truth': 20, 'unmatched_predictions': 0},
}
...
def test_no_surveillance_or_identification_language():
    # Same negation-aware scan as test_propertywatch_demo.py's own Phase
    # 74 guard (issue #89's explicit self-review requirement): a negated
    # mention is the disclaimer itself, never a violation - only an
    # unnegated (positive) claim counts.
    negation_cues = ('not a ', 'not an ', 'never a ', 'never an ', 'nor ', 'no ')
    forbidden = ['face recognition', 'facial recognition', 'identify individuals',
                 'person identification', 'biometric', 'surveillance-identification',
                 'certified', 'guarantees security', 'prevents intrusion', 'autonomous',
...
def test_independent_metrics_match_construction_targets():
    ...
    for task, expected in EXPECTED.items():
        ...
        assert set(result['per_class']) == TASK_LABELS[task]
...
def test_api_evaluate_matches_independently_computed_values(client):
    ...
    for task, config_id in TASK_CONFIGS.items():
        ...
        assert set(result['details']['per_class']) == TASK_LABELS[task]
...
def test_api_rejects_evaluation_without_explicit_parameters(client):
    _seed_demo(client)
    resp = client.post(f'/api/sessions/{SESSION_ID}/evaluate', json={
        'task': 'entrance_package_detection', 'evaluator_type': 'object_detection',
    })
