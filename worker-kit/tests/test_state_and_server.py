"""Pure-logic tests for state.py + server.py - zero third-party
dependency. Drives a real HTTP server on a random local port and makes
real GET requests. Moved here verbatim from the (byte-identical) copies
in yolo_worker's/emotion_worker's own test suites (issue #141) - see
this package's README for why the duplication was reversed."""
import json
import threading
import urllib.error
...
def test_snapshot_before_any_frame_reports_starting():
    state = SharedState(sensor_id='face_rgb')
...
def test_record_frame_updates_the_snapshot():
    state = SharedState(sensor_id='face_rgb')
    state.record_frame(123.0, [{'label': 'happiness', 'confidence': 0.85, 'bbox': {'x': 0, 'y': 0, 'width': 0.3, 'height': 0.4}}])
    snapshot = state.snapshot()
    ...
    assert latest_payload == {'sensor_id': 'face_rgb', 'frame_timestamp_ms': 123.0, 'detections': snapshot.detections}
...
def test_latest_endpoint_serves_the_current_snapshot():
    state = SharedState(sensor_id='face_rgb')
    state.record_frame(42.0, [{'label': 'neutral', 'confidence': 0.7, 'bbox': {'x': 0, 'y': 0, 'width': 0.2, 'height': 0.2}}])
    ...
    try:
        ...
        assert status == 200
        assert payload['sensor_id'] == 'face_rgb'
        assert payload['frame_timestamp_ms'] == 42.0
        assert payload['detections'][0]['label'] == 'neutral'
    finally:
        server.shutdown()
