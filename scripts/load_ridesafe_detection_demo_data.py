#!/usr/bin/env python3
"""Loads examples/profiles/ridesafe-detection-demo-data.json into a
running MultiSens backend via its ordinary REST API, and runs /evaluate
for both object-detection tasks - the v0.8 RideSafe detection demo
(Phase 87). Same "no dedicated import endpoint" reasoning and idempotent-
ish "skip if the session already exists" behavior as every prior loader
script.
...
DATA_PATH = Path(__file__).parent.parent / 'examples' / 'profiles' / 'ridesafe-detection-demo-data.json'
...
def main() -> None:
    ...
    for task in dataset['tasks']:
        ...
        m = results[0]['metrics']
        print(
            f"{task:32s} config={results[0]['configuration_id']:26s} "
